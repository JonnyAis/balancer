...
def get_credentials():
    ...
    if not consumer_key or not consumer_secret:
        ...
        if sandbox:
            if not consumer_key:
                missing_vars.append("ETRADE_SANDBOX_CONSUMER_KEY")
            if not consumer_secret:
                missing_vars.append("ETRADE_SANDBOX_CONSUMER_SECRET")
        else:
            if not consumer_key:
                missing_vars.append("ETRADE_PROD_CONSUMER_KEY")
            if not consumer_secret:
                missing_vars.append("ETRADE_PROD_CONSUMER_SECRET")
...
def start_session(sandbox_override=None):
    ...
    for attempt in range(max_retries):
        try:
            ...
            req_token, req_secret = svc.get_request_token(
            ...
            verifier = input(
                "\n[Auth] Enter verification code (or 'retry' to get new token): "
            ).strip()

            if verifier.lower() == "retry":
                print("[Auth] Retrying with new request token...")
                continue

            if not verifier:
                raise ValueError("No verification code provided")

            print("[Auth] Exchanging verification code for access token...")
            session = svc.get_auth_session(
                req_token, req_secret, params={"oauth_verifier": verifier}
            )
            ...
            if attempt < max_retries - 1:
                print("[Auth] Waiting 10 seconds before retry...")
                time.sleep(10)
            else:
                print("[Auth] All attempts failed")
                raise
...
def renew_access_token(sandbox: bool, access_token: str, access_secret: str):
    ...
    try:
        ...
        r = session.get(
            f"{base_url}/oauth/renew_access_token", params={"oauth_token": access_token}, timeout=8
        )
